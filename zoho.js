...
(async () => {
  // Launch the browser with additional flags for GitHub Actions
  const browser = await launch({
    headless: true,
  ...
  // Navigate to the Zoho login page with retries
  await retryAction(() => page.goto(ZOHO_SIGNIN_PAGE, { waitUntil: "networkidle2", timeout: 60000 }));
  ...
  // Wait for the page to navigate after login with increased timeout
